	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (mEndTime - mStartTime < mLongClickTime) {
				...
				if (startY - event.getY() > 100) {// 上移超过一定距离取消录制， 取消录制
					mProgressAni.cancel();
					onLongClickListener.onCancelListener();
				} else if (mProgressAni != null && mProgressAni.getCurrentPlayTime() / 1000 < mMinTime && !isMaxTime) {
