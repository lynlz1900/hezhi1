...
import com.wukong.hezhi.adapter.AddressReceiptManageAdapter;
...
public class AddressManageActivity extends BaseActivity{
	...
	public String pageName = ContextUtil.getString(R.string.address_manage);
	public String pageCode ="address_manage";
	...
	private AddressReceiptManageAdapter addressReceiptManageAdapter;
	...
	@Override
	protected void init() {
		headView.setTitleStr(ContextUtil.getString(R.string.address_manage));
		initView();
		setListener();
	}
	...
	protected void onDestroy() {
		// TODO Auto-generated method stub
		super.onDestroy();
		AddressManager.getInstance().clean();;
	}
	...
	private void setAdapter(){
		addressReceiptManageAdapter = new AddressReceiptManageAdapter
				(addressReceiptInfosSel,mActivity);
		lv_scroll.setAdapter(addressReceiptManageAdapter);
	...
	public void onClick(View v) {
		...
		case R.id.btn_address_add:
			toActivity(AddressAddActivity.class,Constant.Extra.ISFROM_ADDRESSMANAGE, true);
			break;
		}
	}
		...
		HttpManager.getInstance().post(url, map, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> arg0) {
				...
				if (info != null && info.getHttpCode().equals(HttpCode.SUCESS)) {
					List<AddressReceiptInfo> receiptInfos = new ArrayList<AddressReceiptInfo>();
					try {
						AddressReceiptInfos infos = (AddressReceiptInfos)info.getBody();
						receiptInfos = infos.getDatalist();
					} catch (Exception e) {
					}
					...
					addressReceiptInfosSel.clear();
					addressReceiptInfosSel.addAll(addressReceiptInfos);
					addressReceiptManageAdapter.notifyDataSetChanged();
