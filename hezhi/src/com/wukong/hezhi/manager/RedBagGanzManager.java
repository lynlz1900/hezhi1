...
public class RedBagGanzManager {
    ...
    public static String AR_MODEL_VIDEO = "视频";
    public static String AR_MODEL_U3D = "U3D";

    private int REDBAG_DELAY_TIME = 5000;// 红包延迟弹出时间，默认5秒

    private RedBagGanzManager() {
    }

    private static class Holder {
        private static final RedBagGanzManager SINGLETON = new RedBagGanzManager();
    }

    /**
     * 单一实例
     */
    public static RedBagGanzManager getInstance(Activity activity) {
        mActivity = activity;
     ...
     * 检查是否有红包
     */
    public void checkRedBag(String ppId, String modelName) {
        bugWindows = RedBagWindows.getInstance();
        if (TextUtils.isEmpty(ppId)) {
            return;
        }
        if (AR_MODEL_VIDEO.equals(modelName)) {// 如果是视频
            REDBAG_DELAY_TIME = 12000;
        } else if (AR_MODEL_U3D.equals(modelName)) {// 如果是U3D
            REDBAG_DELAY_TIME = 5000;
        }
        ppid = ppId;
        check();
    }
    ...
    private void check() {
        String URL = HttpURL.URL1 + HttpURL.CHECK_ACTIVITY_GANZ + "perceptionPictureId=" + ppid;
    ...
    public void checkRedBag(String jsonStr) {
        ...
        if (info != null && info.getHttpCode().equals(HttpCode.SUCESS)) {
            ...
            if (AR_MODEL_VIDEO.equals(unityInfo.getModelName())) {// 如果是视频
                REDBAG_DELAY_TIME = 12000;
            } else if (AR_MODEL_U3D.equals(unityInfo.getModelName())) {// 如果是U3D
                REDBAG_DELAY_TIME = 5000;
            }
        ...
        HttpManager.getInstance().post(URL, map, new RequestCallBack<String>() {
            ...
            public void onSuccess(ResponseInfo<String> responseInfo) {
                ...
                if (info != null) {
                    if (info.getHttpCode().equals(HttpCode.SUCESS)) {
                        RedBagInfo redBagInfo = (RedBagInfo) info.getBody();
                        redBagInfo.setRegbagType(GANZ_REDBAG);
                        redBagInfo.setPpid(ppid);
                        ObserveManager.getInstance().notifyState(LiveFragment.class, RedBagGanzManager.class, ppid);// 如果领取成功了，则通过品牌直播界面刷新成已领取。
                        redBagInfo.setAr(true);// 来自unity
     ...
     * 弹框提示
     */
    protected void showTipDialog(String title, final int type) {
        new CustomAlterDialog.Builder(mActivity).setMsg(title)
